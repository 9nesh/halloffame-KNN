...
vector<int> Find_K_Nearest_Neighbors(const Instance& new_instance, const vector<Instance>& training_examples, const vector<Attribute>& attribute_list, int K){
	//initalize our two vectors that we'll be keeping track of to be the size of how many neighbors we need (K)
	vector<int> neighbor_indicies(K, -1);
	vector<double> neighbor_distances(K, 100000000000000);
	
	//begin by loooping through all instances
	for(int i = 0; i < training_examples.size(); i++){
		//compute the distance between the new instance and each training data instance
...
int main(){
  ...
  for(int i = 0; i < test_examples.size(); i++){
	 ...
	 for(int j = 0; j < neighbors_classifications.size(); j++){
	  ...
	  if(prediction != test_examples[i].Get_Nominal_Value(attribute_list_test.size() - 1)){
		  ...
		  cout << "Instance " << i << " in the test data was misclassified as a " << prediction << ". It was really a " <<  test_examples[i].Get_Nominal_Value(attribute_list_test.size() - 1) << endl;
  ...
  //and display it 
  cout << "Error Rate : " << error_rate << " (" << error_rate * 100 << "%)" << endl;
  cout << hof_misclassified << " out of " << incorrect << " misclassifications were actually HOFs." <<endl;
